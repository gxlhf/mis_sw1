...
public class PDF2SWFUtil {
    ...
    public static synchronized void pdf2swf(String fileDir, String exePath) throws IOException {
        ...
        //文件名，不带后缀
         ...
         filePath=filePath.replace("upload","preview");    //转换路径
         fileName=fileName.replace('#','_');
         String version="flashversion=9";     //不然pdf即使转换成功也不会被flexpaper识别并显示
        Process pro = null;
        if (isWindowsSystem()) {
            //如果是windows系统
              //命令行命令
              String cmd = exePath + " \"" + fileDir + "\" -s \"" + version+"\" -o \"" + filePath + "/" + fileName + ".swf\"";
            //Runtime执行后返回创建的进程对象
  ...
  /*  public static void main(String[] args) {
        ...
        try {
            PDF2SWFUtil.pdf2swf("F:\\upload\\ddd.pdf", exePath);
        } catch (IOException e) {
